...
특히, 파이썬에서 데이터가 변수나 리스트 속 요소 안에 직접 담기는 것이 아니라
변수나 요소가 해당 데이터를 참조하기 때문에 이 부분을 고려하고 코드를 짜는 것이 어려웠다.
예컨데 a= [1, 1, 1, 1, 4, 6, 7, 21, 1] 이 선언되었다고 하자.
여기서 1을 담은 a 속의 여러 요소들은 각각 '1'이라는 정수 데이터를 담고 있는 것이 아니라,
요소들은 '1'이라는 정수 데이터가 저장된 메모리의 특정 주소를 참조하고 있다.
육안으로 a를 볼때 각각의 1들이 존재하는 것처럼 보이지만, 실제로 1들은 하나의 1이다.
앞선 예시를 이 문제에 적용해본다면, 특정 1을 제대로 지정하는 것이 관건이다.
만약 큐a 속 인덱스3의 1을 선택한다고 할 때, 인덱스 3번의 1은 몇번 째에 뽑히는 건가.
이를 분명히 하기 위해서 여러 방법을 생각해보다가 해당 1을 절대로 쓰이지 않는 값 0으로 저장하기로 했다.
